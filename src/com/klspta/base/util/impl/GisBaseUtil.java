...
public class GisBaseUtil implements IGisBaseUtil {
    ...
    public static IGisBaseUtil getInstance(String key) throws Exception {
        if (!key.equals("NEW WITH UTIL FACTORY!")) {
            throw new Exception("请从UtilFacoory获取工具.");
        }
        return new GisBaseUtil();
    }
    ...
    private String getPolygon(Polygon polygon) {
        ...
        StringBuffer sb = new StringBuffer(
                "{\"geometryType\":\"esriGeometryPolygon\",\"geometries\":[{\"rings\":[[");
        Point p = null;
        for (int i = 0; i < ri; i++) {
    ...
    private String projectRequest(Polygon polygon) {
        ...
        sb.append("/project?f=json&inSR=").append(local_map_srid).append("&outSR=").append(target_map_srid)
                .append("&geometries=").append(getPolygon(polygon));
