...
public final class ChangeCoordsSysUtil implements IChangeCoordsSysUtil {

    public static void main(String[] args) {
        ChangeCoordsSysUtil c = new ChangeCoordsSysUtil();
        Point point = new Point( 117.804731,34.119006);
//        point = c.changeMe(point);
        System.out.println("转换前84BL："+point.getX()+","+point.getY());
        point=c.changeMe(point, IChangeCoordsSysUtil.BL80_TO_PLAIN80);
        System.out.println(point.getX());
        System.out.println(point.getY());
    }

    private ChangeCoordsSysUtil() {
    }

    public static IChangeCoordsSysUtil getInstance(String key) throws Exception {
        if (!key.equals("NEW WITH UTIL FACTORY!")) {
            throw new Exception();
        }
        return new ChangeCoordsSysUtil();
    }
    ...
    @Override
    public Point changeMe(Point point, String changetype) {
        if(IChangeCoordsSysUtil.GPS84_TO_BALIN80.equals(changetype)){
            this.point84 = point;
        ...
        // 从84BL转换为80BL
        if(IChangeCoordsSysUtil.BL84_TO_BL80.equals(changetype)){
            this.point84 = point;
            //changeDFMToD();//将gps的度分秒格式转换成度
        ...
        if(IChangeCoordsSysUtil.BL80_TO_PLAIN80.equals(changetype)){
            this.point84 = point;
            changeBLToPlain("xian80", true);//将度转换成米
            return this.point84;
        }
        if(IChangeCoordsSysUtil.PLAIN80_TO_BL80.equals(changetype)){
        	point84 = GaussToBL(point.getX(), point.getY());
            return this.point84;
        }
        if(IChangeCoordsSysUtil.PLAIN84_TO_PLAIN80.equals(changetype))
        {
        	this.point84=point;
        	Plain84ToPlain80();
        	return this.point80;
        } if(IChangeCoordsSysUtil.GRID.equals(changetype))
        {
    ...
    private void Plain84ToPlain80() {
        ...
        double yy = m_dDeltaNorth + m_dKvalue * Math.cos(m_dAlpha) * myy + m_dKvalue * Math.sin(m_dAlpha) * myx;
        double xx = m_dDeltaEast - m_dKvalue * Math.sin(m_dAlpha) * myy + m_dKvalue * Math.cos(m_dAlpha) * myx;
     ...
     * <br>Date:2012-4-2
     * @param point
     * @param coordinateType：指定坐标参考，分别为：WGS84 、beijing54 、xian80
     * @return
     */
    public static Point GaussToBL(double X, double Y){//, double *longitude, double *latitude)
      ...
      a=6378137.0; f=0.00335281006247; //80年西安坐标系参数
      ZoneWide = 3; ////6度带宽
