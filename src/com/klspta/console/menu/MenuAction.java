...
public class MenuAction extends AbstractBaseBean{
	...
	{	
		...
		try {
			...
			MenuBean oldMenuBean=ManagerFactory.getMenuManager().getMenuBeanByMenuId(menuBean.getMenuId());
	 ...
	 * @throws Exception
	 */
	private MenuBean getMenuBeanByRequest(HttpServletRequest request) throws Exception
	{
		...
		{
			menuType=ManagerFactory.getMenuManager().getMenuBeanByMenuId(mb.getParentId()).getMenuType();
		...
		String iconPath="";
		try {
			iconPath=new String(request.getParameter("icon").getBytes("ISO-8859-1"),"utf-8");
