...
public class AzfjsHandler extends AbstractBaseBean {
	...
	public void getAzfjsByQuarter() throws UnsupportedEncodingException{
		...
		String sql = "select * from plan安置房建设 t where t.nd=? and t.jd=? and t.xmmc=?";
	...
	public void deleteExist() throws UnsupportedEncodingException{
		...
		String quarter = request.getParameter("quarter");
		String xmmc = new String(request.getParameter("xmmc").getBytes("iso-8859-1"), "utf-8");
		String sql = "delete from plan安置房建设 t where t.nd = ? and t.jd= ? and t.xmmc=?";
