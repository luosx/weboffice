...
public class StatisReport extends AbstractBaseBean {
	...
	static {
		/*
		 * for(int i=0;i<array.length;i++){ array[i]=(i+2013)+""; }
		 */
	}
	 ...
	 * 
	 * <br>
	 * Description:根据参数调用不同的获取报表数据的方法 <br>
	 * Author:王雷 <br>
	 * Date:2013-8-27
	 */
	public void getReportData() {
		String xml = request.getParameter("xml");
		String lbxx = request.getParameter("lbxx");
		if(lbxx != null){
			lbxx = UtilFactory.getStrUtil().unescape(lbxx);
		}
		try {
	...
	private void getEsfzj(String lbxx) {
		String[] names = { "租金" };
		...
		// String sql1 = "select avg(k.czfjj) as zj from esf_zsxx k where
		// k.year='"+year+"' and k.month=? ";
		// generateReportESF(names,colors,alias,array1,array2,sql1,"esfzj","Line");
		generateReport(names, colors, alias, array1, array2, months, "esfzj","Line", "esfzjqknd_pjzj",lbxx);
	}

	/**
	 * 
	 * <br>
	 * Description:二手房租金售价分析图 <br>
	 * Author:李国明 <br>
	 * Date:2013-12-12
	 */
	private void getEsfsj(String lbxx) {
		String[] names = { "售价" };
		String[] colors = { "yellow" };
		String[] alias = { "sj" };
		String[] column = new String[12];
		String[] months = new String[] { "YY", "EY", "SY", "SIY", "WY", "LY",
		...
		String year = Calendar.getInstance().get(Calendar.YEAR) + "";
//		String sql1 = "select avg(k.esfjj) as sj  from esf_zsxx k where k.year='"
//				+ year + "' and k.month=? ";
//		generateReportESF(names, colors, alias, array1, array2, sql1, "esfsj",
//				"Line");
		generateReportSJ(names, colors, alias, array1, array2, months, "esfsj",
				"Line","esfzjqknd_pjlm",lbxx);
	}

	/**
	 * 
	 * <br>
	 * Description:写字楼租金售价分析图 <br>
	 * Author:李国明 <br>
	 * Date:2013-12-12
	 */
	private void getXzlzj(String lbxx) {
		String[] names = { "租金", "保本点" };
		...
		generateReport(names, colors, alias, array1, array2, months, "xzlzj",
				"Line", "xzlzjqknd_pjzj",lbxx);
	}

	/**
	 * 
	 * <br>
	 * Description:获取写字楼租金及售价报表数据 <br>
	 * Author:李国明 <br>
	 * Date:2013-12-10
	 */
	private void getXzlsj(String lbxx) {
		String[] names = { "售价" };
		String[] colors = { "yellow" };
		String[] alias = { "sj" };
		String[] column = new String[12];
		...
		generateReportSJ(names, colors, alias, array1, array2, months, "xzlsj",
				"Line","xzlzjqknd_pjlm",lbxx);
	}

	/**
	 * 
	 * <br>
	 * Description:获取住宅征收规模报表数据 <br>
	 * Author:王雷 <br>
	 * Date:2013-8-27
	 */
	...
	private void getGygm() {
		...
		for (int i = 0; i < names.length; i++) {
			...
			for (int j = 0; j < array.length; j++) {
				...
				e1.setAttribute("y", map.get(alias[i]) == null ? "0" : map.get(
						alias[i]).toString());
				e.addContent(e1);
			}
			data.addContent(e);
	...
	private void getZjsyqk() {
		...
		generateReport(names, colors, alias, sql1, sql2, "ndsr", "bqzmye",
				"zjsyqkfx", "Line");
	}
	...
	private void getZjfx() {
		...
		String sql2 = "select sum(t.bqzmye) bqzmye from hx_sx t where t.nd=?";
		generateReport(names, colors, alias, sql1, sql2, "fzwzjlrgm", "bqzmye",
				"zjfxfx", "Line");
	}
	...
	private void generateReport(String[] names, String[] colors,
			String[] alias, String sql1, String xml, String type) {
		List<Object> allList = new ArrayList<Object>();
		List<Map<String, Object>> list1 = null;
		for (int i = 0; i < array.length; i++) {
			list1 = query(sql1, YW, new Object[] { array[i] });
	 ...
	 * Date:2013-12-12
	 */
	private void generateReportESF(String[] names, String[] colors,
			String[] alias, int[] array1, int[] array2, String sql1,
		...
		if (array1[0] == 1) {
			for (int j = 0; j < array1.length; j++) {
				...
				e1.setAttribute("y", map.get(alias[0]) == null ? "0" : map.get(
						alias[0]).toString());
				e.addContent(e1);
			}
		} else {
			...
			String xml, String type, String tablename,String lbxx) 
	{
		...
		if (array1[0] == 1) {
			...
			sql1 += "avg(" + column[11] + ") as " + column[11] + " from "
					+ tablename + " where rq=? and YW_GUID in ("+sql_w+")";
			listzj1 = query(sql1, YW, new Object[] { year + "" });
			allListzj.add(listzj1);
		} else {
			for (int i = 0; i < array1.length - 1; i++) {
				sql1 += "avg(" + column[array1[i] - 1] + ") as "
						+ column[array1[i] - 1] + ",";
			}
			sql1 += "avg(" + column[array1[array1.length - 1] - 1] + ") as "
					+ column[array1[array1.length - 1] - 1] + " from "
					+ tablename + " where rq=? and YW_GUID in ("+sql_w+")";
			listzj1 = query(sql1, YW, new Object[] { year - 1 + "" });
			allListzj.add(listzj1);
			for (int i = 0; i < array2.length - 1; i++) {
				sql2 += "avg(" + column[array2[i] - 1] + ") as "
						+ column[array2[i] - 1] + ",";
			}
			sql2 += "avg(" + column[array2[array2.length - 1] - 1] + ") as "
					+ column[array2[array2.length - 1] - 1] + " from "
					+ tablename + " where rq=? and YW_GUID in ("+sql_w+")";
		...
		if (array2 == null) {
			for (int j = 0; j < array1.length; j++) {
				...
				e1.setAttribute("name", year + "年" + array1[j] + "月");
				e1.setAttribute("y",
						map.get(column[array1[j] - 1]) == null ? "0" : map.get(
								column[array1[j] - 1]).toString());
				e.addContent(e1);
			}
		} else {
			...
			for (int j = 0; j < array2.length; j++) {
				...
				e1.setAttribute("y",
						map.get(column[array2[j] - 1]) == null ? "0" : map.get(
								column[array2[j] - 1]).toString());
				e.addContent(e1);
			}
		}
		data.addContent(e);
		if (names.length > 1) {
			...
			String sql = "select sum(j.kfcb) as kfcb ,round(sum(j.kfcb)/sum(j.jzgm),2) as lmcb ,round(sum(j.kfcb)/sum(j.jsyd),2)*10000 as dmcb from jc_jiben j where j.ssqy in(?,?,?)";
			result = query(sql, YW, new Object[] { "产业功能改造区", "民生改善区",
					"城市形象提升区" });

			map.put("KFCB", result.get(0).get("KFCB") == null ? "0" : result
					.get(0).get("KFCB").toString());
			map.put("LMCB", result.get(0).get("LMCB") == null ? "0" : result
					.get(0).get("LMCB").toString());
			map.put("DMCB", result.get(0).get("DMCB") == null ? "0" : result
					.get(0).get("DMCB").toString());
			...
			if (array2 == null) {
				for (int j = 0; j < array1.length; j++) {
					...
					e1 = new Element("point");
					e1.setAttribute("name", year + "年" + array1[j] + "月");
					e1.setAttribute("y", map.get("bbd") == null ? "0" : map
							.get("bbd").toString());
					e.addContent(e1);
				}
			} else {
	 ...
	 * 
	 * <br>
	 * Description: 写字楼售价 <br>
	 * Author:李国明 <br>
	 * Date:2013-12-12
	 */
	private void generateReportSJ(String[] names, String[] colors,
			...
			String xml, String type ,String tablename,String lbxx) 
	{
		...
		if (array1[0] == 1) {
			...
			sql1 += "avg(" + column[11] + ") as " + column[11]
					+ " from "+tablename+" where rq=? and YW_GUID in ("+sql_w+")";
			...
			sql1 += "avg(" + column[array1[array1.length - 1] - 1] + ") as "
					+ column[array1[array1.length - 1] - 1]
					+ " from "+tablename+" where rq=? and YW_GUID in ("+sql_w+")";
			...
			sql2 += "avg(" + column[array2[array2.length - 1] - 1] + ") as "
					+ column[array2[array2.length - 1] - 1]
					+ " from "+tablename+" where rq=? and YW_GUID in ("+sql_w+")";
	 ...
	 */
	private void generateReport(String[] names, String[] colors,
			String[] alias, String sql1, String sql2, String property1,
			String property2, String xml, String type) {
		List<Object> allList = new ArrayList<Object>();
		List<Map<String, Object>> list1 = null;
		List<Map<String, Object>> list2 = null;
		for (int i = 0; i < array.length; i++) {
			...
			Map<String, Object> map1 = list1.get(0);
			Map<String, Object> map2 = list2.get(0);
			double d = Double.parseDouble(map1.get(property1) == null ? "0"
					: map1.get(property1).toString())
					+ Double.parseDouble(map2.get(property2) == null ? "0"
							: map2.get(property2).toString());
	 ...
	 * 
	 * <br>
	 * Description:根据传入的文件名解析对应的xml并返回值 <br>
	 * Author:王雷 <br>
	 * Date:2013-8-27
	 * 
	 * @param fileName
	...
	private List<Object> parseXml(String fileName) {
		...
		String classPath = Thread.currentThread().getContextClassLoader()
				.getResource("/").getPath();
		String filePath = classPath.substring(0, classPath
		...
		Element data = root.getChild("charts").getChild("chart").getChild(
				"data");
		if (data != null) {
			data.removeContent();
		}
