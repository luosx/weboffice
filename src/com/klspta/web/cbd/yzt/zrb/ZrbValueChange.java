...
public class ZrbValueChange extends AbstractBaseBean  {
	...
	private String sql = " select round((sum(t.zzcqgm)+sum(t.fzzcqgm))/10000,2) as zzsgm, round(sum(t.zzcqgm)/10000,2) as zzzsgm, round(sum(t.yjhs),2) as zzzshs ,  round(sum(t.zzcqgm)/sum(t.yjhs),0) as hjmj, round(sum(t.fzzcqgm)/10000,2) as fzzzsgm    from jc_ziran t where t.zrbbh like ?";
