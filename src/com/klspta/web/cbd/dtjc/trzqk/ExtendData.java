...
public class ExtendData extends AbstractBaseBean {
	...
	/**
	 * 
	 * <br>Description:获取特定年度和季度的供地体量成本总和
	 * <br>Author:黎春行
	 * <br>Date:2013-11-8
	 * @param nd
	 * @param jd
	...
	public String getgdcbTotal(String nd, String jd){
		...
		List<Map<String, Object>> totalList = query(sql, YW, new Object[]{nd, jd});
		if(totalList.size() > 0){
	...
	/**
	 * 
	 * <br>Description:获取特定年度和季度的供地体量收益总和
	 * <br>Author:黎春行
	 * <br>Date:2013-11-8
	 * @param nd
	 * @param jd
	...
	/**
	 * 
	 * <br>Description:获取项目的评估土地价值、抵押率、融资损失参数
	 * <br>Author:黎春行
	 * <br>Date:2013-11-11
	 * @return
	 */
	public Map<String, String> getParameters(){
		...
		for(Map parameterMap : parametersList){
			if("xm_pgtdjz".equals(String.valueOf(parameterMap.get("key")))){
				parameters.put("pgtdjz", String.valueOf(parameterMap.get("value")));
			}else if("xm_dyl".equals(String.valueOf(parameterMap.get("key")))){
				parameters.put("dyl", String.valueOf(parameterMap.get("value")));
			}else if("xm_rzss".equals(String.valueOf(parameterMap.get("key")))){
				parameters.put("rzss", String.valueOf(parameterMap.get("value")));
			}
		}
		return parameters;
	}
	...
	/**
	 * 
	 * <br>Description:获取特定时间段内的本期融资需求
	 * <br>Author:黎春行
	 * <br>Date:2013-11-13
	 * @param minYear
	 * @param maxYear
	...
	public Map<String, String> getBqrzxq(int minYear, int maxYear){
		...
		List<Map<String, Object>> recordList = query(sql, YW, new Object[]{minYear, maxYear});
	...
	public void addRzxq(String year, String quarter, String value){
		...
		sql.append(year).append("' as nd,'").append(quarter).append("' as jd from hx_sx t ) t2 on (j.nd=t2.nd and j.jd = t2.jd) when matched then ");		
		sql.append("update set j.bqrzxq=round(to_number(?),2) when not matched then insert(j.nd, j.jd, j.bqrzxq)");
	...
	/**
	 * 
	 * <br>Description:获取特定时间段内的权益性资金注入
	 * <br>Author:黎春行
	 * <br>Date:2013-11-13
	 * @param minYear
	 * @param maxYear
	...
	public void addQyxzjzr(String year, String quarter, String value){
		...
		sql.append("merge into hx_sx j using(select distinct '");
		sql.append(year).append("' as nd,'").append(quarter).append("' as jd from hx_sx t ) t2 on (j.nd=t2.nd and j.jd = t2.jd) when matched then ");		
		sql.append("update set j.qyxzjzr=round(to_number(?),2) when not matched then insert(j.nd, j.jd, j.qyxzjzr)");
		sql.append(" values(?, ?, round(to_number(?),2))");
		update(sql.toString(), YW, new Object[]{value, year, quarter, value});
	}
