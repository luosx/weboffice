...
public class XxfkManager extends AbstractBaseBean {
	...
	public void saveOneXxfk(){
		...
		if("new".equals(enterFlag)){//新增
			...
			String insertSql = "insert into wtfkb(yw_guid,wtfkdw,wtfkr,wtfkr_id,wtfkms,wtfksj,wtfklx) values(?,?,?,?,?,?,?)";
			try {
				update(insertSql, YW, new Object[]{yw_guid,wtfkdw,wtfkr,wtfkr_id,wtfkms,wtfksj,"问题反馈"});
	...
	public void getAllXxfk(){
		...
		sqlBuffer.append("select t.yw_guid,t.wtfkdw,t.wtfkr,t.wtfkms,t.wtfksj,t.wtjdxq,t.wtjdry,t.wtjdsj,t.wtfkzt from wtfkb t where 1=1 and t.wtfklx = '问题反馈'");
	...
	public void getOneXxfk(){
		...
		if(getList.size() == 0){//说明是新增记录
			...
			map.put("YW_GUID", new UID().toString().replaceAll(":", "-"));
			map.put("WTFKDW", "");
	...
	public void saveOneYjjy(){
		...
		if("new".equals(enterFlag)){//新增
			...
			String insertSql = "insert into wtfkb(yw_guid,wtfkdw,wtfkr,wtfkr_id,wtfkms,wtfksj,wtfklx) values(?,?,?,?,?,?,?)";
			try {
				update(insertSql, YW, new Object[]{yw_guid,wtfkdw,wtfkr,wtfkr_id,wtfkms,wtfksj,"意见建议"});
				response.getWriter().write("{success:true,msg:true}");
			} catch (Exception e) {
				try {
					response.getWriter().write("{failure:true,msg:true}");
	...
	public void getAllYjjy(){
		String keyword = request.getParameter("keyword");
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer.append("select t.yw_guid,t.wtfkdw,t.wtfkr,t.wtfkms,t.wtfksj,t.wtjdxq,t.wtjdry,t.wtjdsj,t.wtfkzt from wtfkb t where 1=1 and t.wtfklx = '意见建议'");
		//sqlBuffer.append("select t.* from wtfkb t where 1=1");
		if (keyword != null) {
			keyword = UtilFactory.getStrUtil().unescape(keyword);
			sqlBuffer.append(" and").append("(upper(wtfkdw)||upper(wtfkr)||upper(wtfkms)||upper(wtjdxq)||upper(wtjdry)||upper(wtfkzt)").append(" like '%");
