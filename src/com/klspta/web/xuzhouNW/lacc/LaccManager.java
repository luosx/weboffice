...
public class LaccManager extends AbstractBaseBean {
	...
	public void getProcessList() {
		...
		String fullName = UtilFactory.getStrUtil().unescape(
	...
	public void getProcessListXf() {
		...
		List<Map<String, Object>> taskList = query(sql_tasklist, WORKFLOW,
				new String[] { userName });
		...
		if (keyWord != null) {
			keyWord = UtilFactory.getStrUtil().unescape(keyWord);
			sql += " and (upper(t2.ayzt)||upper(t2.ajly)||upper(t1.yw_guid)  like '%"
					+ keyWord + "%')";
		}
	...
	public void saveBhAy(){
        ...
        String sql = "update cljdcpb set ay='"+ ay +"', bh='"+ bh +"',lasj=to_date('"+ sjfgjzrq +"', 'YYYY-MM-DD HH24:MI:SS') where yw_guid = ?";
        update(sql,YW, new Object[]{yw_guid});
        
        String sql2 = "update jacpb set ay=?, bh=? ,lasj=to_date('"+ sjfgjzrq +"', 'YYYY-MM-DD HH24:MI:SS') where yw_guid = ?";
