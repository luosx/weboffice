...
/**
 * 
 * <br>Title:REST入口三层框架之一，用于提供数据库连接
...
public abstract class AbstractDataBaseSupport{
    ...
    public List<Map<String, Object>> query(Class<?> c, String type){
    	String sql = UtilFactory.getConfigUtil().getSQL(c.getName());
    	return query(sql, type);
    }

    public List<Map<String, Object>> query(Class<?> c, String type, String sqlext){
        String sql = UtilFactory.getConfigUtil().getSQL(c.getName() + sqlext);
        return query(sql, type);
    }

    public List<Map<String, Object>> query(Class<?> c, String type, Object[] args){
    	String sql = UtilFactory.getConfigUtil().getSQL(c.getName());
    	return query(sql, type, args);
    }
    
    public List<Map<String, Object>> query(Class<?> c, String type, Object[] args, String sqlext){
    	String sql = UtilFactory.getConfigUtil().getSQL(c.getName() + sqlext);
    	return query(sql, type, args);
    }
    ...
    public void update(Class<?> c, String type){
    	String sql = UtilFactory.getConfigUtil().getSQL(c.getName());
    	update(sql, type);
    }
    
    public void update(Class<?> c, String type, String sqlext){
    	String sql = UtilFactory.getConfigUtil().getSQL(c.getName() + sqlext);
    	update(sql, type);
    }
    
    public void update(Class<?> c, String type, Object[] args){
    	String sql = UtilFactory.getConfigUtil().getSQL(c.getName());
    	update(sql, type, args);
    }
    
    public void update(Class<?> c, String type, Object[] args, String sqlext){
    	String sql = UtilFactory.getConfigUtil().getSQL(c.getName() + sqlext);
    	update(sql, type, args);
    }
...
final class ContextHelper { 
    ...
    static { 
        _ctx = new FileSystemXmlApplicationContext("file:"+Thread.currentThread().getContextClassLoader().getResource("").getPath()+"com/klspta/base/sql/ApplicationContext-jdbc.xml"); 
    } 
