...
public class CoordinateWSDLImp extends AbstractBaseBean implements ICoordinateWSDL {
    ...
    public void parseShpFile(String shpAbsolutePath,String dbfAbsolutePath){
        ...
        try {
	    	...
	    	UtilFactory.getZIPUtil().zip(UtilFactory.getConfigUtil().getShapefileTempPathFloder()+"\\"+currentTime+".zip", UtilFactory.getConfigUtil().getShapefileTempPathFloder()+currentTime);
		} catch (MalformedURLException e) {
			e.printStackTrace();
	...
	public void createShp(String multiPolygonPoint,String shpAbsolutePath,String outPath) {
		...
		try {
			...
			final SimpleFeatureType TYPE3 = DataUtilities.createType("Location","location:Polygon,"+nameAndType);
			...
			ShapefileFeatureLocking featureSource = (ShapefileFeatureLocking) newDataStore.getFeatureSource(typeName);
			// 创建一个事务
