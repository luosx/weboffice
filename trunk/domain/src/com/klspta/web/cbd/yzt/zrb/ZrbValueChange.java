...
public class ZrbValueChange extends AbstractBaseBean  {
	...
	//private String sql = " select round((sum(t.zzcqgm)+sum(t.fzzcqgm))/10000,2) as zzsgm, round(sum(t.zzcqgm)/10000,2) as zzzsgm, round(sum(t.yjhs),2) as zzzshs ,  CEIL(decode(sum(t.yjhs),0,0,(sum(t.zzcqgm)/sum(t.yjhs)))) as hjmj, round(sum(t.fzzcqgm)/10000,2) as fzzzsgm    from jc_ziran t where t.zrbbh like ?";
	private String sql = " select round((sum(t.zzcqgm)+sum(t.fzzcqgm))/10000,2) as zzsgm, round(sum(t.zzcqgm)/10000,2) as zzzsgm, round(sum(t.yjhs),2) as zzzshs , case when sum(t.yjhs)=0 then 0 else round(sum(t.zzcqgm)/sum(t.yjhs),0) end as hjmj, round(sum(t.fzzcqgm)/10000,2) as fzzzsgm    from jc_ziran t where t.zrbbh like ?";
