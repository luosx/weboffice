...
public class YearPlanKgl extends AbstractBaseBean implements IDataClass{
        ...
        if (list.size() > 0) {
            ...
            List<Map<String, Object>> trList;
                ...
                for (int t = 0; t < count; t++) {
                    sb.append("(select  sum(t.kg)  from hx_azf t  where nd =?) as ").append("s").append(t)
                            .append(",");
                    objs[t] = list.get(t).get("nd");
                }
                sb.append("(select  sum(t.kg)  from hx_azf t ) as ").append("s").append(count);
                sb.append(" from dual");
