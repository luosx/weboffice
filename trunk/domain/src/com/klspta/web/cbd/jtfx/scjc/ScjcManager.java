...
public class ScjcManager extends AbstractBaseBean {
	...
	public String getList(String keyord){
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (xqlb.equals("老旧房")) {
				result
						.append("<tr tr class='trsingle' onclick='showMap(this); return false;' ondblclick='editMap(this); return false;' id=row"
								+ i
								+ "><td class='tr02'>"
								+ rownum
								+ "</td><td class='tr02'>"
	...
	public void save() {
		String yw_guid = request.getParameter("yw_guid");
		String ssqy = UtilFactory.getStrUtil().unescape(
				request.getParameter("ssqy"));
		String xqmc = UtilFactory.getStrUtil().unescape(
				request.getParameter("xqmc"));
		String xqlb = UtilFactory.getStrUtil().unescape(
				request.getParameter("xqlb"));
		String bz = UtilFactory.getStrUtil().unescape(
				request.getParameter("bz"));
		if (!yw_guid.equals("") && yw_guid != null) {
			String update = "update esf_jbxx set ssqy='" + ssqy + "',xqmc='"
					+ xqmc + "',xqlb='" + xqlb + "',bz='" + bz
					+ "'where yw_guid=?";
			...
			this.update(insertSql, YW, new Object[] { ssqy, xqmc, xqlb, bz,
					format });
	...
	public String getList2() {
		...
		if (list.size() > 0) {
			...
			for (int j = 0; j < query.size(); j++) {
				...
				update(insert, YW, new Object[] { query.get(j).get("yw_guid"),
						"1", "2014" });
			}
		...
		if (sj != null && !sj.equals("")) {
			...
			for (int i = 0; i < date.length; i++) {
				...
				if (xh.equals("1")) {
					...
					this
							.update(upda, YW, new Object[] { yw_guid, month,
									year });
				}
				if (xh.equals("2")) {
					type = "esfjj";
					if (month.equals("1")) {
						...
						List<Map<String, Object>> list2 = this.query(sql2, YW,
								new Object[] { yw_guid,
	...
	public void months_mm() {
		...
		List<Map<String, Object>> list = query(sql, YW, new Object[] { year,
				month });
		String newTable = "";
		if (list.size() > 0) {
			newTable = getNewTable(list);
			...
			// 插入完成之后，
			String sql2 = "select t.xqmc,t2.zl,t2.esfjj,t2.czl,t2.czfjj,t.yw_guid  from esf_jbxx t ,esf_zsxx t2 where t.yw_guid=t2.yw_guid  and t2.year=? and t2.month=? order by t.dateflag desc";
			List<Map<String, Object>> list2 = query(sql2, YW, new Object[] {
					year, month });
			newTable = getNewTable(list2);
	...
	public void query_year_month() {
		String year = request.getParameter("year");
		String month = request.getParameter("month");
		StringBuffer buffer = new CBDReportManager().getReport("ESFQK",
				new Object[] { year, month, "true" }, "1000px");
