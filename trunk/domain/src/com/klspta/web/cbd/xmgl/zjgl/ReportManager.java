...
public class ReportManager extends AbstractBaseBean{
	 ...
	 * @return
	 */
	public String getReport_view(String yw_guid, String year ,String rolename) {
		StringBuffer  buffer = new StringBuffer();
		StringBuffer title = ZjglBuild.buildTitle(year);
		buffer.append(title);
		String sql_zc = "select distinct t.yw_guid ,t.tree_id, t.tree_name,t.rq,t.parent_id,t.leval  from xmzjgl_zc t where t.yw_guid=? and t.rq=?  order by t.parent_id, t.tree_name";
		String sql_lr =  "select distinct t.yw_guid ,t.tree_id, t.tree_name,t.rq,t.parent_id,t.leval  from xmzjgl_lr t where t.yw_guid=? and t.rq=?  order by t.parent_id, t.tree_name";
		buffer.append(buildTreeBuffer_view(getBeanList(yw_guid, year, sql_lr), "0", rolename, "LR").toString());
		buffer.append(buildTreeBuffer_view(getBeanList(yw_guid, year, sql_zc), "0", rolename, "ZC").toString());
		buffer.append(ZjglBuild.buildSum());
		return buffer.toString();
	}
	...
	public StringBuffer buildTreeBuffer_view(List<XmzjglTreeBean> treeList, String key,String rolename, String type){
		StringBuffer stringBuffer = new StringBuffer();
		for(int i = 0; i < treeList.size(); i++){
			XmzjglTreeBean xmBean = treeList.get(i);
			if(xmBean.getParent_id().equals(key)){
				StringBuffer childString = buildTreeBuffer_view(treeList, xmBean.getTree_id(),rolename, type); 
