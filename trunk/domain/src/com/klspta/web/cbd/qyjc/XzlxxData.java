...
public class XzlxxData  extends AbstractBaseBean implements Runnable{
	
	private static final String formName = "XZLXX";
...
private static final ZrbValueChange linkChange = new ZrbValueChange();
    ...
    private static XzlxxData xzlxxData;
    public static XzlxxData getInstance(){
    	if(xzlxxData == null){
    		xzlxxData = new XzlxxData();
    	}
    	return xzlxxData;
    }
	 /**
     ...
     * @throws Exception 
     */
    public boolean recordGIS(String xzlmc, String polygons) throws Exception{
        ...
        try {
            ...
            boolean isExit = isExit(form_gis, "XZLMC", xzlmc, GIS);
            String sql = "";
            if(isExit){
            	sql = "update " + form_gis + " t set t.SHAPE=sde.st_geometry ('" + wkt + "', " + srid + ") where t.XZLMC='" + xzlmc + "'";
            }else{
                sql = "INSERT INTO "+ form_gis+"(OBJECTID,XZLMC,SHAPE) VALUES ((select nvl(max(OBJECTID)+1,1) from "+form_gis+"),'"
                	+ xzlmc + "',sde.st_geometry ('" + wkt + "', " + srid + "))";
            }
            update(sql, GIS);
