...
public class EsfData  extends AbstractBaseBean implements Runnable{
	
	private static final String formName = "ESF_JBXX";
    private static final String form_gis = "CBD_ESF";
...
private static final ZrbValueChange linkChange = new ZrbValueChange();
    ...
    private static EsfData esfData;
    public static EsfData getInstance(){
    	if(esfData == null){
    		esfData = new EsfData();
    	}
    	return esfData;
	 ...
	 /**
     * 
     * <br>Description:上图，写字楼信息保存到空间库中
     * <br>Author:李国明
     * <br>Date:2013-12-10
     * @param tbbh
     * @param polygon
     ...
     * @throws Exception 
     */
    public boolean recordGIS(String xqmc, String polygons) throws Exception{
...
//            ring.putPoint(p2);
        ...
        try {
            ...
            boolean isExit = isExit(form_gis, "XQMC", xqmc, GIS);
            String sql = "";
            if(isExit){
            	sql = "update " + form_gis + " t set t.SHAPE=sde.st_geometry ('" + wkt + "', " + srid + ") where t.XQMC='" + xqmc + "'";
            }else{
                sql = "INSERT INTO "+ form_gis+"(OBJECTID,XQMC,SHAPE) VALUES ((select nvl(max(OBJECTID)+1,1) from "+form_gis+"),'"
                	+ xqmc + "',sde.st_geometry ('" + wkt + "', " + srid + "))";
            }
            update(sql, GIS);
