...
Ext.onReady(function(){
	putClientCommond("planeManager","getAllList");
    ...
    height=document.body.clientHeight *0.9;
        ...
        columns: [
            ...
            {header: '遥感前时相', dataIndex:'QSX', width: width*0.18, sortable: true,renderer:changKeyword},
            {header: '遥感后时相', dataIndex:'HSX', width: width*0.18, sortable: true,renderer:changKeyword},
            {header: '卫片年度', dataIndex:'YEAR', width: 0, sortable: true,renderer:changKeyword},
            //{header: '核查状态', dataIndex:'ISHF', width: width*0.07, sortable: true,renderer:changKeyword},
            {header: '分析情况', dataIndex:'ID', width: width*0.10, sortable: false,renderer:view}
        ], 
        tbar:[
...
function query(){
	...
	grid.reconfigure(store, new Ext.grid.ColumnModel([
            ...
            {header: '政区名称', dataIndex:'XZQHMC', width: width*0.15, sortable: true,renderer:changKeyword},
            {header: '监测面积', dataIndex:'TBMJ', width: width*0.1, sortable: true,renderer:changKeyword},
            {header: '前时相特征', dataIndex:'QSX', width: width*0.18, sortable: true,renderer:changKeyword},
            {header: '后时相特征', dataIndex:'HSX', width: width*0.18, sortable: true,renderer:changKeyword},
            {header: '卫片年度', dataIndex:'WPLD', width: width*0.15, sortable: true,renderer:changKeyword},
            //{header: '核查状态', dataIndex:'ISHF', width: width*0.07, sortable: true,renderer:changKeyword},
            {header: '分析情况', dataIndex:'ID', width: width*0.10, sortable: false,renderer:view}
	        ]));
	//重新绑定分页工具栏
