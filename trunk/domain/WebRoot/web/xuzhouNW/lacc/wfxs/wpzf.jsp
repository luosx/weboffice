...
Ext.onReady(function(){
	putClientCommond("lacc","getWPdata");
    ...
    height=document.body.clientHeight - 10;
        ...
        columns: [
            ...
            {header: '监测面积', dataIndex:'TBMJ', width: width*0.2, sortable: true},
            {header: '前时相特征', dataIndex:'QSX', width: width*0.2, sortable: true},
            {header: '后时相特征', dataIndex:'HSX', width: width*0.2, sortable: true},
            //{header: '自动分析结果', dataIndex:'ISHF', width: width*0.15, sortable: true},
            {header: '查看', dataIndex:'ROWNUM1', width: width*0.12, sortable: false,renderer:view}
        ], 
	 ...
	 for(var i=0;i<records.length;i++){
      ...
      Ext.Msg.confirm("请确认","是否要判定为合法案件", function(button,text){   
                if(button=="yes"){//这个是通过yes or no 来确定的点击值 
                	var path = "<%=basePath%>";
					putClientCommond("anjianManager","changeWpStatus");
   					putRestParameter("ids",ids);
					var result = restRequest();
    				if(result == '0'){
    					Ext.Msg.alert('提示该案件已判定为合法案件!');  
    					document.location.reload();  
    				}
    				else{
...
function sendTask(){
  window.showModalDialog("<%=basePath%>web/default/ajgl/xiafa/xiafa.jsp","","dialogWidth=650px;dialogHeight=450px;status=no;scroll=no");
  //  window.open("<%=basePath%>web/default/ajgl/xiafa/xiafa.jsp");
}
    ...
    if(grid.getSelectionModel().hasSelection()){
	 ...
	 for(var i=0;i<records.length;i++){
	   if(i==records.length-1){
	    ...
	    putRestParameter("ajstatus","3");
	    restRequest();
	    document.location.reload();
	  }else{
	    Ext.Msg.alert('提示','请选择卫片！');
	  }
  }
  
  
function linkTo(){
	document.location.href="<%=basePath%>web/default/wpzf/qbtb_table/qbtb_main.jsp";
}
function query(){
var zqbm=document.getElementById('zqbm').value;
var type=document.getElementById('type').value;
document.location.href="<%=basePath%>web/default/wpzf/nw_wptask/wpzfjc_dkxxb.jsp?zqbm="+zqbm+"&hczt=<%=hczt%>&type="+type+"&year=<%=year%>&zfjcType=<%=zfjcType%>";
}
...
grid.reconfigure(store,new Ext.grid.ColumnModel([
           ...
           {header: '图斑编号', dataIndex:'TBBH', width: width*0.12, sortable: true,renderer:changKeyword},
            ...
            {header: '后时相特征', dataIndex:'HSX', width: width*0.2, sortable: true},
            //{header: '自动分析结果', dataIndex:'ISHF', width: width*0.15, sortable: true},
            {header: '分析结果', dataIndex:'ROWNUM1', width: width*0.12, sortable: false,renderer:view}
        ]));
//重新绑定分页工具栏
