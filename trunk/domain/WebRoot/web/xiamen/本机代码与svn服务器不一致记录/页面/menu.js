...
//window.open(basePath+"model/fxgiscomponents/mapPrint/print.jsp","print",'width='+w+',height='+(h+120)+',top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');	
}

//图斑查询
function findFeature(serviceid,layerid,where,fields){
frames["lower"].swfobject.getObjectById("FxGIS").findFeature(serviceid,layerid,where,fields);
}
...
frames["lower"].swfobject.getObjectById("FxGIS").setCenterAt(xpoint,ypoint);
}
...
function tbQuery(){
	frames["lower"].swfobject.getObjectById("FxGIS").panmap();
	parent.Ext.getCmp('east-panel').expand();
	parent.Ext.getCmp('east-panel').setTitle('卫片图斑查询');
