...
input,img {
	vertical-align: middle;
		...
		function initComponent(){	
		 ...
		 putClientCommond("xuzhouWWManager","getxzqhData");
...
grid = new Ext.grid.GridPanel({
        ...
        columns:[
            ...
            {header: '负责人',dataIndex:'CAR_PERSON',width: width*0.11, sortable: true,renderer:changKeyword},
            {header: '联系电话',dataIndex:'CAR_PERSON_PHONE', width: width*0.1, sortable: true,renderer:changKeyword},
            {header: '车牌号',dataIndex:'CAR_NUMBER', width: width*0.11, sortable: false,renderer:changKeyword},
            {header: '行政区', dataIndex:'CAR_INFO_XZQH_NAME',width: width*0.14, sortable: false,renderer:changKeyword},
            {header: '车型号',dataIndex:'CAR_STYLE', width: width*0.11, sortable: false,renderer:changKeyword},
            {header: '购买日期', dataIndex:'CAR_GMRQ',width: width*0.11, sortable: false,renderer:changKeyword},
 ...
 updateForm=new Ext.form.FormPanel({
	   ...
	   baseCls: 'x-plain',
       labelWidth:80,	
       url:"<%=basePath%>/service/rest/xuzhouWWManager/addCarData",
       width:330, 
 ...
 Ext.onReady(function(){
 	///var myData = ajaxRequest("<%=basePath%>","xuzhouWWManager","getAllCarData","");
 	putClientCommond("xuzhouWWManager","getAllCarData");
	myData = restRequest();
    initComponent();    	   	 
       })
...
function del(){
           return "<a href='#' onclick='delCar();return false;'><img src='base/gis/images/delete.png' alt='删除'></a>";
         }
         
function modify(){
			flag = "modify";
          return "<a href='#' onclick='modifyCar();return false;'><img src='base/gis/images/conf.png' alt='修改'></a>";
         }
...
//编码处理
  ...
  Ext.MessageBox.confirm('注意', '删除后不能恢复，您确定吗？',function(btn){
   if(btn=='yes'){
	    var path = "<%=basePath%>";
	    var actionName = "xuzhouWWManager";
...
function query(){
    ...
    putClientCommond("xuzhouWWManager","queryCarData");
 ...
 grid.reconfigure(store, new Ext.grid.ColumnModel([
           {header: '车辆名称',dataIndex:'CAR_NAME', width: width*0.1, sortable: true},
