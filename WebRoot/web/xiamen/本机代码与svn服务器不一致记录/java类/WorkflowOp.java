...
public class WorkflowOp extends AbstractBaseBean implements IWorkflowOp {
	...
	public List<Map<String, Object>> getAllRoles(String nodeName, String wfId, String userId) {
		...
		//得到当前登录人员的角色，并得到当前登录人员所属角色同级的角色
		List<Role> userRole = null;
		try {
			userRole = ManagerFactory.getRoleManager().getRoleWithUserID(userId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		String sql1 = "select c.roleid from core_roles c where c.parentroleid =(select t.parentroleid from core_roles t where t.roleid=?)";
        List<Map<String,Object>> childernList = query(sql1,CORE,new Object[]{userRole.get(0).getRoleid()});
        //根据不同节点过滤Roles
        List<Map<String, Object>> returnRoles = new ArrayList<Map<String, Object>>();
		if (nodeName.trim().equals("分局执法监察科处理")) {
			for (int i = 0; i < childernList.size(); i++) {
				for (int j = 0; j < Roles.size(); j++) {
					if(((String)childernList.get(i).get("roleid")).equals(Roles.get(j).get("roleid"))){
						returnRoles.add(Roles.get(j));
					}
				}
			}
		}else if(nodeName.trim().equals("国土所处理")){
			for (int i = 0; i < childernList.size(); i++) {
				for (int j = 0; j < Roles.size(); j++) {
					if(((String)childernList.get(i).get("roleid")).equals(Roles.get(j).get("roleid"))){
						returnRoles.add(Roles.get(j));
		...
		}else if(nodeName.trim().equals("分局相关科室处理")){
			for (int i = 0; i < childernList.size(); i++) {
				for (int j = 0; j < Roles.size(); j++) {
					if(((String)childernList.get(i).get("roleid")).equals(Roles.get(j).get("roleid"))){
						returnRoles.add(Roles.get(j));
		...
		}else if(nodeName.trim().equals("上报分局执法监察科")){
			for (int i = 0; i < childernList.size(); i++) {
				for (int j = 0; j < Roles.size(); j++) {
					if(((String)childernList.get(i).get("roleid")).equals(Roles.get(j).get("roleid"))){
						returnRoles.add(Roles.get(j));
