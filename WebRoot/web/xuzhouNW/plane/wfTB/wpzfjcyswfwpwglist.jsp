...
Ext.onReady(function(){
	putClientCommond("planeManager","getwpwgList");
    ...
    var height=document.body.clientHeight*0.95 ;
        ...
        columns: [
            ...
            {header: '政区名称', dataIndex:'XZQHMC', width: width*0.20, sortable: true,renderer:changKeyword},
            {header: '图斑面积', dataIndex:'TBMJ', width: width*0.08, sortable: true,renderer:changKeyword},
            {header: '前时相', dataIndex:'QSX', width: width*0.12, sortable: true,renderer:changKeyword},
            {header: '后时相', dataIndex:'HSX', width: width*0.12, sortable: true,renderer:changKeyword},
            {header: '卫片时间', dataIndex:'WPLD', width: width*0.15, sortable: true,renderer:changKeyword},
            {header: '下发情况', dataIndex:'XIAFA', width: width*0.1,  sortable: true,renderer:changKeyword},
...
function query(){
	...
	grid.reconfigure(store, new Ext.grid.ColumnModel([
	       sm,
            ...
            {header: '政区名称', dataIndex:'XZQHMC', width: width*0.20, sortable: true,renderer:changKeyword},
            {header: '图斑面积', dataIndex:'TBMJ', width: width*0.08, sortable: true,renderer:changKeyword},
            {header: '前时相特征', dataIndex:'QSX', width: width*0.12, sortable: true,renderer:changKeyword},
            {header: '后时相特征', dataIndex:'HSX', width: width*0.12, sortable: true,renderer:changKeyword},
            {header: '卫片时间', dataIndex:'WPLD', width: width*0.15, sortable: true,renderer:changKeyword},
            {header: '下发情况', dataIndex:'XIAFA', width: width*0.1,  sortable: true,renderer:changKeyword},
            {header: '分析情况', dataIndex:'ID', width: width*0.06, sortable: false,renderer:view}
	        ]));
