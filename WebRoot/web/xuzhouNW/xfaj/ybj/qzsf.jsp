...
Ext.onReady(function(){
   putClientCommond("xfaj","getQueryFinishList");
   putRestParameter("fullName",escape(escape("<%=fullName%>")));
   putRestParameter("tabtp",escape(escape("群众上报")));
	myData = restRequest();
    ...
    var height=document.body.clientHeight - 10;
        grid = new Ext.grid.GridPanel({
          title:'信访案件已办结（群众上报）列表',
        store: store,
        sm:sm,
        columns: [
        	new Ext.grid.RowNumberer(),
        	{header: '文号', dataIndex:'WH', width: width*0.12, sortable: true,renderer:changKeyword},
            {header: '县市区', dataIndex:'JBDZ', width: width*0.10, sortable: true,renderer:changKeyword},
            {header: '来文时间', dataIndex:'LWSJ', width: width*0.1, sortable: true,renderer:changKeyword},
            {header: '来文机关', dataIndex:'LWJG', width: width*0.1, sortable: true,renderer:changKeyword},
            {header: '类型', dataIndex:'JBLX', width: width*0.1, sortable: true,renderer:changKeyword},
            {header: '承办部门', dataIndex:'CBBM_1', width: width*0.1, sortable: true,renderer:changKeyword},
            {header: '承办人员', dataIndex:'CBRY', width: width*0.12, sortable: true,renderer:changKeyword},
            {header: '办结情况', dataIndex:'BJQK', width: width*0.12, sortable: true,renderer:changKeyword},
            {header: '查看', dataIndex:'INDEX', width: width*0.10, sortable: false,renderer:pro}
        ], 
        tbar:[
