...
Ext.onReady(function(){
    ...
    grid = new Ext.grid.GridPanel({
        ...
        columns: [
            ...
            {header: '登记人', dataIndex:'DJR', width: width*0.08, sortable: false,renderer:changKeyword},
            {header: '被举报单位', dataIndex:'BJBDW', width: width*0.13, sortable: false},
            {header: '登记日期', dataIndex:'DJRQ', width: width*0.10, sortable: false},
            {header: '问题发生地', dataIndex:'WTFSD7', width: width*0.13, sortable: false},
            {header: '线索摘要', dataIndex:'XSZY', width: width*0.17, sortable: false},
            ...
            {header: '查看', dataIndex:'ROWNUM1', width: width*0.07, sortable: false, renderer:view},
...
function lian(id){
	...
	Ext.Msg.confirm("请确认","确定要立案吗？", function(button,text){ 
                if(button=="yes"){//这个是通过yes or no 来确定的点击值 
	                ...
	                parent.location.href="<%=basePath%>"+path.urlPath+"&returnPath=<%=basePath%>web/xuzhouNW/ajgl/xfjb/xfclzTab.jsp";
                } 
            });  
}
function xinxi(id){
