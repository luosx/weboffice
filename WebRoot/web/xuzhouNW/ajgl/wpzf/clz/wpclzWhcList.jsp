...
Ext.onReady(function(){
    ...
    var height=document.body.clientHeight ;
        ...
        columns: [
            ...
            {header: '监测面积', dataIndex:'TBMJ', width: width*0.22, sortable: true},
            {header: '前时相特征', dataIndex:'QSX', width: width*0.21, sortable: true},
            {header: '后时相特征', dataIndex:'HSX', width: width*0.18, sortable: true},
            {header: '分析结果', dataIndex:'ROWNUM1', width: width*0.08, sortable: false,renderer:view}
		...
		if(rs>=0&&filePath!=""){ 
	      form.getForm().submit({  
	        success: function(form, action){  
	           alert('文件导入成功！,共导入'+action.result.msg+'条卫片成果！');  
...
//详细信息
function view(id){
 return "<a href='#' onclick='viewDetail("+id+");return false;'><img src='base/gis/images/view.png' alt='动分析结果'></a>";
}
function viewDetail(id){
var yw_guid=myData[id].TBBH;
zfjcType=7;
//window.open("<%=basePath%>model/resourcetree/resourceTree.jsp?zfjcType="+zfjcType+"&yw_guid="+yw_guid);
window.open("<%=basePath%>web/xuzhouNW/wpzf/pdaStatus.jsp?yw_guid="+yw_guid);
}
...
grid.reconfigure(store,new Ext.grid.ColumnModel([
            ...
            {header: '监测面积', dataIndex:'TBMJ', width: width*0.22, sortable: true},
            {header: '前时相特征', dataIndex:'QSX', width: width*0.21, sortable: true},
            {header: '后时相特征', dataIndex:'HSX', width: width*0.21, sortable: true},
            {header: '分析结果', dataIndex:'ROWNUM1', width: width*0.08, sortable: false,renderer:view}
