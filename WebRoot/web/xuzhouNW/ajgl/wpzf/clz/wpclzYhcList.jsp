...
Ext.onReady(function(){
    ...
    var height=document.body.clientHeight;
        ...
        columns: [
            ...
            {header: '监测面积', dataIndex:'TBMJ', width: width*0.10, sortable: true},
            {header: '前时相特征', dataIndex:'QSX', width: width*0.18, sortable: true},
            ...
            {header: '查看', dataIndex:'ROWNUM1', width: width*0.08, sortable: false,renderer:view},
...
function lian(id){
	...
	Ext.Msg.confirm("请确认","确定要立案吗？", function(button,text){ 
                if(button=="yes"){//这个是通过yes or no 来确定的点击值 
	                ...
	                //document.location.reload();
	                parent.location.href="<%=basePath%>"+path.urlPath+"&returnPath=<%=basePath%>web/xuzhouNW/ajgl/wpzf/clz/wpclzTab.jsp?year=<%=year%>";
                } 
            }); 
...
grid.reconfigure(store,new Ext.grid.ColumnModel([
            ...
            {header: '监测面积', dataIndex:'TBMJ', width: width*0.16, sortable: true},
            {header: '前时相特征', dataIndex:'QSX', width: width*0.2, sortable: true},
            {header: '后时相特征', dataIndex:'HSX', width: width*0.18, sortable: true},
            {header: '分析结果', dataIndex:'ROWNUM1', width: width*0.08, sortable:false,renderer:fxjg},
            {header: '查看', dataIndex:'ROWNUM1', width: width*0.06, sortable: false,renderer:view},
       		{header: '立案', dataIndex:'ROWNUM1', width: width*0.08, sortable: false, renderer:lianButton}
