...
function lianDetail(id){
	Ext.MessageBox.confirm('确认框', '当前案件将立案处理,确定？',function (btn){
		if(btn == 'yes'){
		    putClientCommond("wpzfHandler","changeStatus");
    		putRestParameter("objectid",id);
			myData = restRequest();		
			//做立案处理
			window.location.href="<%=basePath%>/model/workflow/startWorkflow.jsp?beanId=startWorkflowLacc&zfjcType=90&returnPath=web/xuzhouNW/wpzf/wpzfList/wpzfCLZ/wpzfclz.jsp";
		}
	});
}
