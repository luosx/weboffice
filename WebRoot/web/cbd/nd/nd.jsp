...
<%@page import="com.klspta.web.cbd.nd.Annual" %>
<%
...
<script type="text/javascript">
	function showTable() {
		...
		if (document.getElementById('table3').checked) {
			...
			document.getElementById('gdtl4').style.display = '';
		} else {
			...
			document.getElementById('gdtl4').style.display = 'none';
		}
...
<h2 align="center">北京商务中心区土地储备规划——年度实施计划动态决策</h2>
	...
	<tr style='background:#CCFFFF;' id='azfjs1'>
		...
		<tr style='background: #CCFFFF' id='azfjs3'>
			<td>7</td>
			<td>使用量<br>（万㎡）</td>
			<td><%=anfjsTable[2][0] %></td>
			<td><%=anfjsTable[2][1] %></td>
			<td><%=anfjsTable[2][2] %></td>
			<td><%=anfjsTable[2][3] %></td>
			<td><%=anfjsTable[2][4] %></td>
			<td><%=anfjsTable[2][5] %></td>
			<td><%=anfjsTable[2][6] %></td>
			<td><%=anfjsTable[2][7] %></td>
			<td><%=anfjsTable[2][8] %></td>
			<td><%=anfjsTable[2][9] %></td>
		</tr>
		<tr style='background: #CCFFFF' id='azfjs4'>
			<td>8</td>
			<td>安置房存量<br>（万㎡）</td>
			<td><%=anfjsTable[3][0] %></td>
			<td><%=anfjsTable[3][1] %></td>
			<td><%=anfjsTable[3][2] %></td>
			<td><%=anfjsTable[3][3] %></td>
			<td><%=anfjsTable[3][4] %></td>
			<td><%=anfjsTable[3][5] %></td>
			<td><%=anfjsTable[3][6] %></td>
			<td><%=anfjsTable[3][7] %></td>
			<td><%=anfjsTable[3][8] %></td>
			<td><%=anfjsTable[3][9] %></td>
		</tr>
		<!-- 供地体量 -->
		...
		<tr style='background: #99CC00' id='gdtl4'>
			<td>12</td>
			<td>储备库融资能力<br>（亿元）</td>
			<td><%=gdtlTable[3][0] %></td>
			<td><%=gdtlTable[3][1] %></td>
...
<!-- 供地体量 -->
		<% if(gdtlTable1.length>0){for(int i=0;i<gdtlTable1.length;i++){%>
		<tr id='kf1'>
		<td ><%=i+1%></td>
			<td>规模</td>
	...
	注：1、鉴于企业为主体项目为企业自有资金开发建设，开发实施时序及资金安排较难控制，因此不在本表体现。<br>
   &nbsp;&nbsp; &nbsp;&nbsp; 2、本计划仅考虑上述项目开发实施工作，后续项目可结合本计划具体实施情况陆续启动。
