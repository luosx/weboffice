  	...
  	left = new Ext.tree.TreePanel({
	    ...
	    listeners:{'checkchange':function(node,checked){
			var nodes=left.getChecked();
      		...
      		var url = "<%=basePath%>web/cbd/tjbb/chart.jsp?xml=esfsj.xml&lbxx="+treeTextList;
      		url=encodeURI(url);
      		url=encodeURI(url);
		 	...
		 	treeTextList="";
        		...
        		putRestParameter("tablename","esfzjqknd_pjlm");
    			mydata = restRequest();
...
function init(){
	document.getElementById("center1").innerHTML="<iframe id='xxtj' name='xxtj' style='width: 100%; height: 380px; overflow: auto;' src='<%=basePath%>web/cbd/tjbb/chart.jsp?xml=esfsj.xml&lbxx='></iframe>";
}
</script>
	</head>
	<body onload="init();">
