...
function createBox(t, s,isClose){   
 ...
 if(t){html.push('<h3 class="msg-h3">');   
      html.push(t);   
      html.push('</h3>');   
 }   
 if(isClose){   
    html.push('<span class="msg-close" onclick="Ext.QuickMsg.close()"><img src="" mce_src=""/></span>');   
 }   
 html.push(s);   
 html.push('</div></div></div>');   
...
show : function(title, msg, width, time, alignEl, offsets, position,animate,isClose){                      
  ...
  this.close();   
  if(!msgCt){   
    msgCt = Ext.DomHelper.insertFirst(alignEl, {id:'msg-div'}, true);
