    ...
    var form;
  	Ext.onReady(function(){
		...
		var endtimes=new Ext.form.ComboBox({
  		...
  		var checktimes=new Ext.form.ComboBox({
											emptyText:"==请选择==",
											fieldLabel:"对比时间",
											id:'checktime',
											width:100,
        ...
        items: [{
            ...
            items: [{
				...
				items:[{xtype:'button', text:'查询',icon:'<%=basePath%>base/gis/images/find.png', id:'query', width:50,
					    handler: function() {
											// var str = Ext.getCmp("canton").getValue();
											// str = encodeURIComponent(encodeURIComponent(Ext.encode(str)));
											 var parmeters = "";
											 var condition="";
											 var begin_year = Ext.getCmp("starttime").getValue();
											 var end_year = Ext.getCmp("endtime").getValue();
											 var checktime = Ext.getCmp("checktime").getValue();
                							 //Ext.Msg.alert('信息',str);	
                							 
                							 if(begin_year!=""){
											 	parmeters += "&begin_year="+begin_year;
											 	condition+="condition=where nd>="+begin_year;  
											 }else{
											 	parmeters += "&begin_year=1949";
											 	condition+="condition=where nd>=1949";
											 }
											 
											 if(end_year!=""){
											 	parmeters += "&end_year="+end_year;
											 	condition+=" and nd<="+end_year;  
											 }else{
											 	parmeters += "&end_year=<%=year%>";
											 	condition+=" and nd<=<%=year%>";   
											 }  
											  
											 if(checktime!=""){
											 	parmeters += "&checktime="+checktime;
											 	
											 }else{
											 	parmeters += "&checktime=<%=year%>";
